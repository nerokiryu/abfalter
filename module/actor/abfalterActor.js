...
export default class abfalterActor extends Actor {
    ...
    prepareBaseData() {
        ...
        system.monsterStats.charCombinedCost = Math.floor(
            system.monsterChar.agi.charBaseCostTotal +
                system.monsterChar.con.charBaseCostTotal +
                system.monsterChar.str.charBaseCostTotal +
                system.monsterChar.dex.charBaseCostTotal +
                system.monsterChar.per.charBaseCostTotal +
                system.monsterChar.int.charBaseCostTotal +
                system.monsterChar.pow.charBaseCostTotal +
        ...
        if (!system.metaMagic.remProtection.bought && !system.metaMagic.remProtection2.bought && !system.metaMagic.remProtection3.bought) {
            ...
            (system.metaMagic.remProtection.bought && system.metaMagic.remProtection2.bought) ||
        ...
        if (!system.metaMagic.defExper.bought && !system.metaMagic.defExper2.bought && !system.metaMagic.defExper3.bought) {
            ...
            (system.metaMagic.defExper.bought && system.metaMagic.defExper2.bought) ||
        ...
        if (!system.metaMagic.offExper.bought && !system.metaMagic.offExper2.bought && !system.metaMagic.offExper3.bought) {
            ...
            (system.metaMagic.offExper.bought && system.metaMagic.offExper2.bought) ||
        ...
        if (!system.metaMagic.forceSpeed.bought && !system.metaMagic.forceSpeed2.bought && !system.metaMagic.forceSpeed3.bought) {
            ...
            (system.metaMagic.forceSpeed.bought && system.metaMagic.forceSpeed2.bought) ||
        ...
        if (!system.metaMagic.advZeonRegen.bought && !system.metaMagic.advZeonRegen2.bought && !system.metaMagic.advZeonRegen3.bought) {
            ...
            (system.metaMagic.advZeonRegen.bought && system.metaMagic.advZeonRegen2.bought) ||
        ...
        system.metaMagic.derived.definedProjNumber =
            system.metaMagic.defMagicProj.bought +
            system.metaMagic.defMagicProj2.bought +
            system.metaMagic.defMagicProj3.bought +
            system.metaMagic.defMagicProj4.bought +
            system.metaMagic.defMagicProj5.bought +
            system.metaMagic.defMagicProj6.bought +
            system.metaMagic.defMagicProj7.bought;
        ...
        //mystic concentration
        if (
            !system.metaMagic.spellSpec80.bought &&
            !system.metaMagic.spellSpec70.bought &&
            !system.metaMagic.spellSpec60.bought &&
            !system.metaMagic.spellSpec60x.bought &&
            !system.metaMagic.spellSpec50.bought &&
            !system.metaMagic.spellSpec30.bought &&
    ...
    newSupernaturalShield(shieldLP) {
        this.update({
            system: {
                shield: {
                    value: shieldLP,
    ...
    async applyDamageSupernaturalShield(damage, doubleDamage, newCombatResult) {
        ...
        if (newShieldPoints > 0) {
            ...
            if (newShieldPoints < 0 && newCombatResult) {
                const result = calculateDamage(newCombatResult.attack, 0, newCombatResult.armor, Math.abs(newShieldPoints), newCombatResult.halvedAbsorption);
    ...
    consumeAccumulatedZeon(zeonCost) {
        const newAccumulatedZeon = this.system.maccu.actual - zeonCost;
        const newZeon = this.system.zeon.value - zeonCost;
        ...
        this.update({
            system: {
                maccu: { actual: newAccumulatedZeon },
            },
        });
    }
     ...
     */
    applyDamage(damage) {
        const newLifePoints = this.system.lp.value - damage;

        this.update({
            system: {
                lp: { value: newLifePoints },
            },
        });
    }
    ...
    applyFatigue(fatigueUsed) {
        const newFatigue = this.system.fatigue.value - fatigueUsed;
        this.update({
            system: {
    ...
    accumulateDefenses(keepAccumulating) {
        const defensesCounter = this.getFlag("abfalter", "defensesCounter") || {
            accumulated: 0,
            keepAccumulating,
        };
        const newDefensesCounter = defensesCounter.accumulated + 1;
        if (keepAccumulating) {
            this.setFlag("abfalter", "defensesCounter", {
                accumulated: newDefensesCounter,
                keepAccumulating,
            });
        } else {
            this.setFlag("abfalter", "defensesCounter.keepAccumulating", keepAccumulating);
        }
    }

    /**
     * Resets the accumulated defenses counter for an abfalterActor object.
     *
     * @example
     * const actor = new abfalterActor(data, context);
    ...
    resetDefensesCounter() {
        ...
        if (defensesCounter === undefined) {
            ...
            this.setFlag("abfalter", "defensesCounter.accumulated", 0);
        }
    }
    ...
    prepareDerivedData() {
        ...
        system.lifepoints.base = Math.floor(
            25 + 10 * system.stats.Constitution.final + system.stats.Constitution.mod - Math.ceil((system.stats.Constitution.final - 1) / system.stats.Constitution.final) * 5
        );
        if (system.toggles.dmgRes == false) {
            system.lp.max = Math.floor(
                system.lifepoints.base + system.lifepoints.spec + system.lifepoints.temp + system.lifepoints.bonus + Math.ceil(system.lifepoints.multiple * system.stats.Constitution.final)
            );
        } else {
        ...
        //Ki Accumulation
        system.kiPool.agi.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Agility.kiPoolAccuBase +
                    system.kiPool.agi.spec +
                    system.kiPool.agi.temp +
                    system.kiPool.agi.bonus +
                    system.kiPool.agi.default +
                    Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        system.kiPool.con.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Constitution.kiPoolAccuBase +
                    system.kiPool.con.spec +
                    system.kiPool.con.temp +
                    system.kiPool.con.bonus +
                    system.kiPool.con.default +
                    Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        system.kiPool.dex.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Dexterity.kiPoolAccuBase +
                    system.kiPool.dex.spec +
                    system.kiPool.dex.temp +
                    system.kiPool.dex.bonus +
                    system.kiPool.dex.default +
                    Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        system.kiPool.str.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Strength.kiPoolAccuBase +
                    system.kiPool.str.spec +
                    system.kiPool.str.temp +
                    system.kiPool.str.bonus +
                    system.kiPool.str.default +
                    Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        system.kiPool.pow.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Power.kiPoolAccuBase + system.kiPool.pow.spec + system.kiPool.pow.temp + system.kiPool.pow.bonus + system.kiPool.pow.default + Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        system.kiPool.wp.accumTot = Math.max(
            0,
            Math.floor(
                system.stats.Willpower.kiPoolAccuBase + system.kiPool.wp.spec + system.kiPool.wp.temp + system.kiPool.wp.bonus + system.kiPool.wp.default + Math.min(0, ~~(system.aamField.final / 20))
            )
        );
        ...
        system.kiPool.con.poolTot = Math.floor(
            system.stats.Constitution.kiPoolBase + system.kiPool.con.specMax + system.kiPool.con.tempMax + system.kiPool.dex.bonusMax + system.kiPool.con.defaultMax
        );
        ...
        if (system.toggles.unifiedPools == true) {
            if (system.toggles.innatePower == true) {
                ...
                system.unifiedKi.max = Math.floor(
                    system.kiPool.agi.poolTot +
                        system.kiPool.con.poolTot +
                        system.kiPool.dex.poolTot +
                        system.kiPool.str.poolTot +
                        system.kiPool.pow.poolTot +
                        system.kiPool.wp.poolTot +
                        system.kiPool.unifiedBonus
                );
            }
        }

        //atk, blk, dodge
        system.combatValues.attack.final = Math.floor(
            system.combatValues.attack.base +
                system.combatValues.attack.special +
                system.combatValues.attack.temp +
                system.combatValues.attack.bonus +
                system.stats.Dexterity.mod +
                system.aamField.final
        );
        system.combatValues.block.final = Math.floor(
            system.combatValues.block.base + system.combatValues.block.special + system.combatValues.block.temp + system.combatValues.block.bonus + system.stats.Dexterity.mod + system.aamField.final
        );
        system.combatValues.dodge.final = Math.floor(
            system.combatValues.dodge.base + system.combatValues.dodge.special + system.combatValues.dodge.temp + system.combatValues.dodge.bonus + system.stats.Agility.mod + system.aamField.final
        );
        ...
        if (system.kiAbility.kiEnergyArmor.status == true) {
            //Energy armor add 2 energy AT for free
            system.otherStats.enArm = 2;
            if (system.toggles.greaterEnergyArmor == true && system.kiAbility.kiArcaneArmor.status == false) {
                //Greater energy armor only if arcane is not bought
        ...
        //Resistances
        if (system.kiAbility.kiPhysDom.status == true) {
            //Physical Dominion adds 10 PhR
            system.otherStats.phrDom = 10;
        } else {
            system.otherStats.phrDom = 0;
        }
        if (system.kiAbility.nemiBodyEmpty.status == true) {
            //Physical Dominion adds 20 PhR
        ...
        //Movement
        system.movement.final = Math.floor(
            system.stats.Agility.final +
                system.movement.spec +
                system.movement.temp +
                system.movement.bonus +
                system.movement.sizeBase -
                system.movement.pen +
                Math.min(0, Math.ceil(system.aamField.final / 20)) +
                system.armor.wearArmor.totalMovePen
        );
        ...
        system.initiative.final = Math.floor(
            system.initiative.base +
                system.initiative.class +
                system.initiative.spec +
                system.initiative.bonus +
                system.initiative.kiAbilityBonus +
                ~~system.initiative.wepFinalSpeed -
                system.armor.wearArmor.totalNatPen
        );
        ...
        system.secondaryFields.perceptive.kidetection.baseNotice = Math.floor(
            system.secondaryFields.perceptive.notice.temp +
                system.secondaryFields.perceptive.notice.spec +
                system.secondaryFields.perceptive.notice.base +
                system.secondaryFields.perceptive.notice.classBonus +
                system.secondaryFields.perceptive.notice.natTotal
        );
        ...
        system.secondaryFields.perceptive.kidetection.natTotal = Math.floor(
            system.secondaryFields.perceptive.kidetection.modValue +
                ...
                Math.ceil(system.secondaryFields.perceptive.kidetection.nat * system.secondaryFields.perceptive.kidetection.modValue)
        );
        ...
        system.secondaryFields.perceptive.kidetection.final = Math.floor(
            system.secondaryFields.perceptive.kidetection.base +
                system.secondaryFields.perceptive.kidetection.spec +
                system.secondaryFields.perceptive.kidetection.temp +
                system.secondaryFields.perceptive.kidetection.classBonus +
                system.secondaryFields.perceptive.kidetection.natTotal +
                system.secondaryFields.perceptive.kidetection.bonus +
                system.aamField.final
        );
        ...
        system.secondaryFields.subterfuge.kiconceal.baseHide = Math.floor(
            system.secondaryFields.subterfuge.hide.temp +
                ...
                system.secondaryFields.subterfuge.hide.classBonus +
                system.secondaryFields.subterfuge.hide.natTotal
        );
        ...
        system.secondaryFields.subterfuge.kiconceal.natTotal = Math.floor(
            system.secondaryFields.subterfuge.kiconceal.modValue +
                ...
                Math.ceil(system.secondaryFields.subterfuge.kiconceal.nat * system.secondaryFields.subterfuge.kiconceal.modValue)
        );
        ...
        system.secondaryFields.subterfuge.kiconceal.final = Math.floor(
            system.secondaryFields.subterfuge.kiconceal.base +
                system.secondaryFields.subterfuge.kiconceal.spec +
                system.secondaryFields.subterfuge.kiconceal.temp +
                system.secondaryFields.subterfuge.kiconceal.classBonus +
                system.secondaryFields.subterfuge.kiconceal.natTotal +
                system.secondaryFields.subterfuge.kiconceal.bonus +
                system.aamField.final
        );
        ...
        system.mregen.final = Math.floor(
            (system.maccu.fromPow * system.mregen.regenmult + system.mregen.spec + system.mregen.temp + system.mregen.bonus + system.maccu.finalFull) * system.mregen.recoverymult
        );
        ...
        system.summoning.summon.final = Math.floor(
            system.summoning.summon.base + system.summoning.summon.class + system.summoning.summon.spec + system.summoning.summon.bonus + system.stats.Power.mod + Math.min(0, system.aamField.final)
        );
        system.summoning.control.final = Math.floor(
            system.summoning.control.base +
                system.summoning.control.class +
                system.summoning.control.spec +
                system.summoning.control.bonus +
                system.stats.Willpower.mod +
                Math.min(0, system.aamField.final)
        );
        system.summoning.bind.final = Math.floor(
            system.summoning.bind.base + system.summoning.bind.class + system.summoning.bind.spec + system.summoning.bind.bonus + system.stats.Power.mod + Math.min(0, system.aamField.final)
        );
        system.summoning.banish.final = Math.floor(
            system.summoning.banish.base + system.summoning.banish.class + system.summoning.banish.spec + system.summoning.banish.bonus + system.stats.Power.mod + Math.min(0, system.aamField.final)
        );
